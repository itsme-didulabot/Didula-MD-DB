...
const ssweb = async (options = {}) => {
  try {
    const { data } = await axios.get(`https://api.pikwy.com/?tkn=125&d=3000&u=${options.url}&fs=0&w=1280&h=1200&s=100&z=100&f=${options.extOutput}&rt=jweb`);
    return data;
  } catch (error) {
    console.error(error);
    throw new Error("Failed to capture the screenshot. Please try again.");
  }
};
...
async (conn, mek, m, {
  ...
  try {
    const screenshotUrl = await ssweb({ url: q, extOutput: "png" });
